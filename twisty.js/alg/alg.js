...
(function (name, definition) {
    ...
    function canonicalizeMove(orig, dimension) {
        ...
        if (mKind == "single") {
            ...
            if (dimension % 2 == 1) {
                ...
                // On even cubes M = m
                move.startLayer = (dimension) / 2;
                move.endLayer = (dimension) / 2 + 1;
            }
    ...
    var cube = (function () {
        ...
        function fromString(algString, short = false) {
            if(short){
                algString = toShortNames(algString);
            }else{
                algString = toLongNames(algString);
            }

            return alg_jison.parse(algString);
        }

        function toLongNames(algString) {
            algString = algString.replace(/r/g, "2R");
            algString = algString.replace(/l/g, "2L");
            algString = algString.replace(/u/g, "2U");
            algString = algString.replace(/d/g, "2D");
            algString = algString.replace(/f/g, "2F");
            algString = algString.replace(/b/g, "2B");

            return algString;
        }

        function toShortNames(algString) {
            algString = algString.replace(/2R/g, "r");
            algString = algString.replace(/2L/g, "l");
            algString = algString.replace(/2U/g, "u");
            algString = algString.replace(/2D/g, "d");
            algString = algString.replace(/2F/g, "f");
            algString = algString.replace(/2B/g, "b");
